...
[ ! -z "$debugOn" ] && echo "EOSIO_CLONED_REPO_ROOT: (${EOSIO_CLONED_REPO_ROOT})"
if [ ! -d "$EOSIO_CLONED_REPO_ROOT" ]; then
  echo "I don't see that folder; did you type it correctly?"
  exit 0
fi

[ ! -z "$debugOn" ] && echo "About to move needed files from (${EOSIO_CLONED_REPO_ROOT})"
EOSIO_INSTALL_DIR=/usr/local/eosio
[ ! -z "$debugOn" ] && echo "EOSIO_INSTALL_DIR: (${EOSIO_INSTALL_DIR})"
BOOST_INSTALL_INCLUDE_DIR=${HOME}/opt/boost/include
[ ! -z "$debugOn" ] && echo "BOOST_INSTALL_INCLUDE_DIR: (${BOOST_INSTALL_INCLUDE_DIR})"

echo "Fixing Compilation Paths..."
# sudo cp -R ${EOSIO_CLONED_REPO_ROOT}/include ${EOSIO_INSTALL_DIR}/include/

sudo mkdir -p ${EOSIO_INSTALL_DIR}/include/eosiolib
sudo cp ${EOSIO_CLONED_REPO_ROOT}/contracts/eosiolib/*.h* ${EOSIO_INSTALL_DIR}/include/eosiolib/
